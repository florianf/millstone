    ...
    existsAsync(options.filepath, function(exists) {
        if (exists) {
            ...
            if (path.extname(options.filepath) == '.zip') {
                try {
                  var names = zipper.sync.unzip(options.filepath).memory().contents();
                  if (names.length < 1) {
...
function unzip(file, callback) {
    ...
    var zippedFs;
    try {
        zippedFs = zipper.sync.unzip(file).memory();
    } catch (err) {
        return callback(err);
    }

    if (zippedFs.contents().length < 1) {
        return callback(new Error("could not find any valid data in zip archive: '" + file + "'"));
    }

    var remain = zippedFs.contents().length;
    var ds_files = _(zippedFs.contents()).chain()
    ...
    zippedFs.contents().forEach(function(name) {
        ...
        if (!path.extname(name) || (name.indexOf(search_basename) < 0) || name[0] === '.' || basename[0] === '.') {
            ...
            var buffer = zippedFs.read(name, "buffer");
